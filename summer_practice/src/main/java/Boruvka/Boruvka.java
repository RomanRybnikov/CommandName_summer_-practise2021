        ...
        for(int i = 1;i<size && tree.getEdges().size()<size-1;i*=2){
            for(Edge edge:graph.getEdges()){
                ...
                if(vertex1Parent!=vertex2Parent){//если vertex1 и vertex2 не входят в одну компоненту
                    if(cheapest[graph.getVertexes().indexOf(vertex1Parent)]==null || cheapest[graph.getVertexes().indexOf(vertex1Parent)].getWeight()>weight){//если для этой компоненты это минимальное ребро
                        cheapest[graph.getVertexes().indexOf(vertex1Parent)] = new Edge(vertex1,vertex2,weight);
                    }
                    if(cheapest[graph.getVertexes().indexOf(vertex2Parent)]==null
                            || cheapest[graph.getVertexes().indexOf(vertex2Parent)].getWeight()>weight){
                        cheapest[graph.getVertexes().indexOf(vertex2Parent)] = new Edge(vertex1,vertex2,weight);
                    }
                }
            }
            ...
            for(Integer j:graph.getVertexes()){
                if(cheapest[graph.getVertexes().indexOf(j)]!=null){
                    Edge edge = cheapest[graph.getVertexes().indexOf(j)];
    ...
    class UnionFind{
        ...
        UnionFind(int n){
            ...
            for(Integer i:Boruvka.this.graph.getVertexes()){
                parents[Boruvka.this.graph.getVertexes().indexOf(i)] = i;
                ranks[Boruvka.this.graph.getVertexes().indexOf(i)] = 0;
            }
        }

        public int find(int v){
            while(v != parents[Boruvka.this.graph.getVertexes().indexOf(v)]){
                v=parents[Boruvka.this.graph.getVertexes().indexOf(v)];
            }
            return v;
        }
        ...
        public void union(int v1,int v2){
            ...
            if(ranks[Boruvka.this.graph.getVertexes().indexOf(vertex1Parent)]<ranks[Boruvka.this.graph.getVertexes().indexOf(vertex2Parent)]){
                parents[Boruvka.this.graph.getVertexes().indexOf(vertex1Parent)] = vertex2Parent;
            } else if(ranks[Boruvka.this.graph.getVertexes().indexOf(vertex2Parent)]<ranks[Boruvka.this.graph.getVertexes().indexOf(vertex1Parent)]){
                parents[Boruvka.this.graph.getVertexes().indexOf(vertex2Parent)] = vertex1Parent;
            }else {
                parents[Boruvka.this.graph.getVertexes().indexOf(vertex2Parent)]= vertex1Parent;
                ranks[Boruvka.this.graph.getVertexes().indexOf(vertex1Parent)]++;
            }
        }
    }
}
